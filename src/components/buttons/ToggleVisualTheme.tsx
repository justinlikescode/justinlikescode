...
export default function ToggleVisualTheme({ className }: { className?: string }) {
    ...
    return (
        <div className={`flex items-center justify-center ${className}`}>
            ...
            >
                <span
                    className={`bg-white dark:bg-secondary h-6 w-6 rounded-full transition-all relative ${theme == "light" ? "translate-x-full" : "translate-x-0"}`}
                    aria-hidden="true"
                >
                    <i
                        className={`nf ${theme == "light" ? "nf-fa-cloud_sun text-primary" : "nf-fa-cloud_moon text-darker-900"}`}
                    ></i>
                </span>
            </button>
        </div>
